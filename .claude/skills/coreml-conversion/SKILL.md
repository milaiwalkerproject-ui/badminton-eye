...
description: "Convert and optimize ML models for on-device CoreML deployment. Use when deploying custom ML models on-device, converting PyTorch models with coremltools, compressing models (quantization, palettization, pruning), implementing stateful LLM inference (KV-cache), or optimizing CoreML performance."
---
...
The standard PyTorch → CoreML workflow uses coremltools for tracing and conversion:
...
Three techniques exist, each with different tradeoffs:
